...
function MyApp({ Component, pageProps }) {
  ...
  const addToCart = (itemCode, qty, price, name, size, variant, category) => {
    ...
    if (itemCode in cart) {
      ...
      myCart[itemCode] = { qty: 1, price, name, size, variant, category }
    }
    setCart(myCart);
    saveCart(myCart)
  }

  const removeFromCart = async (itemCode, qty, price, name, size, variant, category) => {
    let myCart = cart;
  ...
  const buyNow = (itemCode, qty, price, name, size, variant, category)=>{
    let newCart = {}
    newCart[itemCode]= {qty: 1, price, name, size, variant, category}
    setCart(newCart)
    saveCart(newCart)
  ...
  return <>
    ...
    />
    <Navbar key={key} logout={logout} user={user} cart={cart} addToCart={addToCart} removeFromCart={removeFromCart} clearCart={clearCart} subTotal={subTotal} totalQty={totalQty} />
    <Component cart={cart} user={user} useremail={useremail} addToCart={addToCart} removeFromCart={removeFromCart} buyNow={buyNow} clearCart={clearCart} subTotal={subTotal} totalQty={totalQty} fAmt={fAmt} {...pageProps} />
    <Footer />
  </>
}
